    ...
    for iname in interfaces:
        if interfaces[iname]:
            ...
            try:
                ...
                logging.exception(f'Error occurred while accessing stats.bytes_sent: {e}')

            try:
                tmp[interface]['bytes_recv'] = stats.bytes_recv
            except Exception as e:
                tmp[interface]['bytes_recv'] = None
                logging.exception(f'Error occurred while accessing stats.bytes_recv: {e}')

            try:
                tmp[interface]['packets_sent'] = stats.packets_sent
            except Exception as e:
                tmp[interface]['packets_sent'] = None
                logging.exception(f'Error occurred while accessing stats.packets_sent: {e}')

            try:
                tmp[interface]['packets_recv'] = stats.packets_recv
            except Exception as e:
                tmp[interface]['packets_recv'] = None
                logging.exception(f'Error occurred while accessing stats.packets_recv: {e}')

            try:
                tmp[interface]['errin'] = stats.errin
            except Exception as e:
                tmp[interface]['errin'] = None
                logging.exception(f'Error occurred while accessing stats.errin: {e}')

            try:
                tmp[interface]['errout'] = stats.errout
            except Exception as e:
                tmp[interface]['errout'] = None
                logging.exception(f'Error occurred while accessing stats.errout: {e}')

            try:
                tmp[interface]['dropin'] = stats.dropin
            except Exception as e:
                tmp[interface]['dropin'] = None
                logging.exception(f'Error occurred while accessing stats.dropin: {e}')

            try:
                tmp[interface]['dropout'] = stats.dropout
            except Exception as e:
                tmp[interface]['dropout'] = None
                logging.exception(f'Error occurred while accessing stats.dropout: {e}')
